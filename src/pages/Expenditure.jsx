...
const Expenditure = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setForm({ votehead: '', amount: '', description: '', year: new Date().getFullYear(), assetAccount: '', localChurch: scopedChurchId, date: new Date().toISOString().split('T')[0] });
      fetchExpenditures();
    } catch (error) {
            ...
            <table className="app-table min-w-full text-sm">
              ...
              <tbody>
                ...
                {expenditures.length === 0 && (
                  <tr>
                    <td colSpan="6" className="px-5 py-12 text-center text-slate-500">No expenditure records found.</td>
                  </tr>
                )}
              </tbody>
            </table>
